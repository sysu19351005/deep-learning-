'''
该文件用来测试模型对图片的超分辨重建效果，需要改变的是图片路径和模型路径
'''
import argparse
import logging
import os
...
def main(args):
    ...
    if args.model_path is not None:
        ...
        model.load_state_dict(torch.load(args.model_path, map_location=torch.device("cuda:0")),strict=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--arch", default="rfb_esrgan", type=str, choices=model_names,
                        help="Model architecture: " +
                             " | ".join(model_names) +
                             ". (Default: `rfb_esrgan`)")
    parser.add_argument("--lr",default="./assets/monkey.png",type=str,required=True,
    ...
    parser.add_argument("--model-path", default="./weights/PSNR-best.pth", type=str,
